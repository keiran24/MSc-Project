...
class MSE:
    ...
    def moving_application(self, df, **kwargs):
        ...
        df_out = pd.DataFrame(columns=columns,index=df.index)
        # Compute MSE for each column using a rolling window
        for col in columns:
            df_out[col] = move_mse(df[col].values,length,df_ref[col].values,nanprop)
        return df_out
...
@nb.njit       
def func(buffer,ref,pctnan):
    diff = buffer - ref
    if np.isnan(diff).sum()/len(diff)*100 >= pctnan:
        return np.nan
    return np.nanmean((diff)**2)

@nb.njit
def update(buffer,xin):
    buffer[0:-1] = buffer[1:]
    buffer[-1] = xin
    return buffer

@nb.njit
def move_mse(data,windowsize,ref,pctnan):
    res = np.full_like(data, np.nan) # init the np.array of results with nans
    buffer = init(data[:windowsize]) # init the buffer with the first window of size windowsize of the data
    res[windowsize-1] = func(buffer,ref,pctnan) # enter the res[halfsize] from applying func on state
    for i in range(windowsize, data.size):
        xin = data[i] # xin = point added to the buffer, xout = point to remove
        buffer = update(buffer, xin) # update the buffer
        res[i] = func(buffer,ref,pctnan) # calc new value 
    return res 